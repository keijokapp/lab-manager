...
export default async function (lab, username) {
	...
	try {
		const id = `${lab._id}:${username}`;
		const rev = uuid();

		await db.doTn(async tn => {
			const existing = await tn.get(['instance', id]);
			if (existing) {
				// eslint-disable-next-line no-throw-literal
				throw 'conflict';
			}

			await tn.set(['instance', id], JSON.stringify(instance));
			await tn.set(['instance', id, 'rev'], rev);
			await tn.set(['instance-token', instance.privateToken], id);
			await tn.set(['instance-token', instance.publicToken], id);
		});
