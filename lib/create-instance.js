...
async function resolveRef(repository, ref) {
	...
	try {
		...
		logger.error('Failed to resolve ref', { respository, ref, e: e.message });
	}
	return null;
}
...
async function listFiles(repository, rev) {
	try {
		...
		logger.error('Failed to list files from repository', { respository, rev, e: e.message });
	}
	return null;
}
...
async function fetchFile(repository, rev, file) {
	try {
		...
		logger.error('Failed to fetch file from repository', { respository, rev, file, e: e.message });
	}
	return null;
}
...
async function lxdCreateMachine(instance, id) {
	...
	try {
		...
		if(instance.lab.machines[id].enable_private) {
			await lxdRequest('/containers/' + encodeURIComponent(instance.machines[id].name)
				+ '/files?path=' + encodeURIComponent('/root/instance.json'), {
				method: 'POST',
...
async function virtualboxCreateMachine(instance, id) {
	...
	try {
		const response = await request.put(config.virtualbox + '/machine/' + encodeURIComponent(instance.machines[id].name), requestData);
		if(response.ok) {
			return true;
		}
...
export default async function(instance) {
	...
	if('assistant' in lab && !('assistant' in instance)) {
		timing.assistant = [Date.now()];
		try {
			const response = await request.post(lab.assistant.url + '/api/v2/labusers', {
				api_key: lab.assistant.key,
				host: config.appUrl,
				username: instance.username,
				fullname: instance.username,
				labID: lab.assistant.lab,
			...
			if(!response.ok) {
				...
				instance.assistant = {
					...
					link: lab.assistant.url + '/lab/' + encodeURIComponent(lab.assistant.lab) + '/' + encodeURIComponent(body.userKey)
				};
			}
		} catch(e) {
	...
	if('endpoints' in lab && 'labProxy' in config) {
		...
		try {
			const response = await request.put(config.labProxy.url + '/api/endpoints/' + encodeURIComponent(instance.privateToken) + '?auth-token=' + encodeURIComponent(config.labProxy.key), { endpoints });
			if(!response.ok) {
		...
		timing.endpoints[1] = Date.now();
	}


	if('gitlab' in lab) {
		timing.gitlab = [Date.now()];
