...
async function createAssistant(instance) {
	...
	try {
		const response = await request.post(instance.lab.assistant.url + '/api/v2/labusers', {
		...
		if(!response.ok) {
			...
			instance.assistant = {
				...
				link: instance.lab.assistant.url + '/lab/' + encodeURIComponent(instance.lab.assistant.lab)
				+ '/' + encodeURIComponent(body.userKey)
			};
		}
	} catch(e) {
...
async function createEndpoints(instance) {
	...
	try {
		const response = await request.put(config.labProxy.url + '/api'
			+ '/endpoints/' + encodeURIComponent(instance.privateToken)
			+ '?auth-token=' + encodeURIComponent(config.labProxy.key), { endpoints });
