...
async function virtualboxCreateMachine(instance, id) {
	...
	try {
		const response = await fetch(config.virtualbox + '/machine/' + encodeURIComponent(instance.machines[id].name), {
			method: 'PUT',
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify(requestData)
		});
...
async function createEndpoints(instance) {
	...
	try {
		const response = await fetch(config.labProxy.url + '/api'
			+ '/endpoints/' + encodeURIComponent(instance.privateToken)
			+ '?auth-token=' + encodeURIComponent(config.labProxy.key), {
			method: 'PUT',
			headers: { 'content-type': 'application/json' },
