...
function MachineLimits({ limits, valueRef }) {
	const [cpu, setCpu] = useState(limits.cpu);
	const [cpuAllowance, setCpuAllowance] = useState(limits.cpuAllowance || 100);
	const [memory, setMemory] = useState(limits.memory);
	...
	useEffect(() => {
		valueRef.current = () => {
			if (cpu || cpuAllowance !== 100 || memory) {
				return {
					cpu: cpu || undefined,
					cpuAllowance: cpuAllowance && cpuAllowance !== 100 ? cpuAllowance : undefined,
				...
				<Table.Row>
					...
					<Table.Cell>
						<input
							...
							onChange={e => setCpuAllowance(+e.target.value)}
						/>
						{`${cpuAllowance}%`}
					</Table.Cell>
				</Table.Row>
...
function MachineRepositories({ repositories: initialRepositories, valueRef }) {
	const [repositories, setRepositories] = useState(initialRepositories);
	const [allRepositories, setAllRepositories] = useState({});
	const [nameErrors, setNameErrors] = useState({});
	const [locationErrors, setLocationErrors] = useState({});
	const [refErrors, setRefErrors] = useState({});
	const [order, setOrder] = useState(initialRepositories.map((n, i) => i));
	const [lastIndex, setLastIndex] = useState(initialRepositories.length);
	...
	function setRepository(i, field) {
		return e => {
			setRepositories({
				...repositories,
				[i]: {
	...
	return (
		<div>
						...
						<Table.Row key={i}>
							...
							<Table.Cell>
								<Input
									...
									onBlur={setRepository(i, 'location')}
									onChange={validateLocation(i)} error={locationErrors[i]}
								/>
							</Table.Cell>
							<Table.Cell>
								<datalist id={`refs-${i}`}>
								...
								<Input
									fluid
									list={`refs-${i}`}
									defaultValue={repositories[i].ref}
									onBlur={setRepository(i, 'ref')}
									onChange={validateRef(i)}
			...
			<Button positive onClick={() => addRepository()} autoFocus>New</Button>
		</div>
	);
}
...
function Machine({
	...
	const containerConfigiration = machine.type === 'lxd' && (
		<Modal
