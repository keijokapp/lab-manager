...
async function loadTemplates() {
	if(needTemplates) {
		needTemplates = false;
		const response = await request.get('machine?templates', null);
		if(response.ok) {
...
async function loadRepositories() {
	if(needRepositories) {
		...
		const response = await request.get('repository', null);
		if(response.ok) {
			response.json().then(body => {
...
export default class extends React.Component {
	...
	async save() {
		...
		request.put('lab/' + encodeURIComponent(this.props.lab._id) + '?rev=' + encodeURIComponent(this.props.lab._rev), this.getValue())
			.then(response => {
				if(response.ok) {
