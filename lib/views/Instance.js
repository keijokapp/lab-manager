...
class Machine extends React.Component {
	...
	setMachineState(state) {
		...
		request.put('instance/' + encodeURIComponent(window.INIT_STATE.instanceToken) + '/machine/' + encodeURIComponent(this.props.id) + '?ip&rev=' + encodeURIComponent(this.props.rev), {
			state
		})
			.then(response => {
				if(response.ok) {
	...
	reload() {
		...
		request.get('instance/' + encodeURIComponent(window.INIT_STATE.instanceToken) + '/machine/' + encodeURIComponent(this.props.id) + '?ip&rev=' + encodeURIComponent(this.props.rev))
			.then(response => {
				if(response.ok) {
					return response.json().then(body => {
						this.setState({
...
export default class extends React.Component {
	...
	deleteInstance() {
		...
		request.delete('lab/' + encodeURIComponent(this.props.instance.lab._id) + '/instance/' + encodeURIComponent(this.props.instance.username) + '?rev=' + encodeURIComponent(this.props.instance._rev))
			.then(response => {
				if(response.ok) {
				...
				<Segment>
					...
					<div>
						<Label size="big">
							...
							{startTime.toDateString()} (<TimeSince date={startTime}/>)
						</Label.Detail>
						</Label>
					</div>
					<div>
