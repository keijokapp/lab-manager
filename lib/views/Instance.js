...
function Repositories(props) {
	...
	for(const id in props.template) {
		repositories.push(<Table.Row key={id}>
			<Table.Cell>{id}</Table.Cell>
			<Table.Cell>{props.template[id].name}</Table.Cell>
			<Table.Cell>{props.template[id].head}</Table.Cell>
			<Table.Cell>{props.repositories[id].link}</Table.Cell>
		</Table.Row>);
	}
...
export default class extends React.Component {
	...
	render() {
		...
		const repositories = <Repositories repositories={instance.repositories} template={lab.repositories}/>;
