...
class Machine extends React.Component {
			...
			.then(response => {
				if (response.ok) {
					return response.json().then(body => {
						this.setState({ machine: body, loading: null });
					});
				}
	...
	render() {
		const { primary, template } = this.props;
		const { machine, loading } = this.state;

		let stateButton;
		if (machine.state === 'poweroff' || machine.state === 'stopped') {
			stateButton = (
				<Button icon primary disabled={!!loading} loading={loading === 'running'} onClick={() => this.setMachineState('running')}>
					<Icon name="bolt" />
					Power on
				</Button>
			);
			...
			if (template.type === 'virtualbox') {
				stateButton = (
					<Button.Group>
						<Button icon color="yellow" disabled={!!loading} loading={loading === 'acpipowerbutton'} onClick={() => this.setMachineState('acpipowerbutton')}>
							<Icon name="power off" />
							Shutdown
						</Button>
						<Button.Or />
						<Button icon negative disabled={!!loading} loading={loading === 'poweroff'} onClick={() => this.setMachineState('poweroff')}>
							<Icon name="plug" />
							Power off
						</Button>
					</Button.Group>
		...
		return (
			<Table.Row>
				...
				<Table.Cell>
					{'networks' in machine && machine.networks.map((network, i) => <div key={i}>{network.name}</div>)}
				</Table.Cell>
...
function Machines({ machineOrder, machines, rev, primary, templates }) {
...
function Repositories(props) {
	const { template, repositories } = props;
		...
		return (
			<Grid>
				<Grid.Column>
					...
					{instance.imported && <Header color="blue" size="medium">Imported</Header>}
					<Segment>
						{lab._id && (
							<div>
								<Label size="big">
