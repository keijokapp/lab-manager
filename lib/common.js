 ...
 */
export async function virtualboxRequest(path, options = {}) {
	...
	return fetch(config.virtualbox.url + path, {
		method: options.method,
		headers: {
...
export async function createGitlabGroup(gitlab, publicToken) {
	try {
		...
		if (body && body.message === 'Failed to save group {:path=>["has already been taken"]}') {
			...
			try {
				const response = await fetch(`${gitlab.url}/api/v4/groups/lab-${encodeURIComponent(publicToken)}?private_token=${encodeURIComponent(gitlab.key)}`, {
...
export async function createGitlabUser(gitlab, publicToken) {
	try {
		...
		if (body && (body.message === 'Email has already been taken' || body.message === 'Username has already been taken')) {
			...
			try {
				const response = await fetch(`${gitlab.url}/api/v4/users?username=user-${encodeURIComponent(publicToken)}&private_token=${encodeURIComponent(gitlab.key)}`, {
					headers: {
						accept: 'application/json',
						'x-request-id': reqid()
					}
 ...
 */
export async function iTeeLabinfo(privateToken) {
	try {
		if (!('iTee' in config)) {
			throw new Error('I-Tee is not configured');
		}

		const response = await fetch(`${config.iTee.url}/labinfo.json?uuid=${encodeURIComponent(privateToken)}`, {
		...
		if (response.ok) {
			...
			if (body instanceof Object) {
				...
				if (body.message === 'Unable to find labuser with given uid'
					|| body.message === 'Unable to find active labuser with given uid') {
					return null;
				}
			}
