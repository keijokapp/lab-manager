...
async function virtualboxDeleteMachine(name) {
	try {
		if('virtualbox' in config) {
			const response = await request.delete(config.virtualbox + '/machine/' + encodeURIComponent(name));
			if(!response.ok) {
				logger.error('Failed to delete machine', {
					type: 'virtualbox',
					machine: name,
 ...
 * @returns {object|undefined} GitLab group object or undefined on error
 */
export async function createGitlabGroup(gitlab, publicToken) {
	try {
		const response = await request.post(gitlab.url + '/api/v4/groups?private_token=' + encodeURIComponent(gitlab.key), {
		...
		if(response.ok) {
			...
			try {
				const response = await request.get(gitlab.url + '/api/v4/groups/' + 'lab-' + encodeURIComponent(publicToken) + '?private_token=' + encodeURIComponent(gitlab.key), null);
				const body = await response.json();
				if(response.ok) {
					return {
						id: body.id,
 ...
 * @param gitlab {object} GitLab instance info
 * @param publicToken {string} instance public token
 * @returns {object|undefined} GitLab user object or undefined on error
 */
export async function createGitlabUser(gitlab, publicToken) {
	try {
		const response = await request.post(gitlab.url + '/api/v4/users?private_token=' + encodeURIComponent(gitlab.key), {
		...
		if(response.ok) {
			...
			try {
				const response = await request.get(gitlab.url + '/api/v4/users?username='
					+ 'user-' + encodeURIComponent(publicToken)
					+ '&private_token=' + encodeURIComponent(gitlab.key), null);
				const body = await response.json();
				if(response.ok) {
					if(body.length === 0) {
 ...
 * @returns {boolean|undefined} true on success, undefined on error
 */
export async function addGitlabUserToGroup(gitlab, group, user) {
	try {
		const response = await request.post(gitlab.url + '/api/v4/groups/' + encodeURIComponent(group.id) + '/members?private_token=' + encodeURIComponent(gitlab.key), {
			user_id: user.id,
			access_level: 30
		});
		if(response.ok) {
...
export async function lxdMachineInfo(name) {
	try {
		...
		const ips = [];
		for(const network in response.network) {
			if(network !== 'lo') {
				for(const address of response.network[network].addresses) {
...
export async function virtualboxMachineInfo(name, ip = false) {
	try {
		...
		const response = await request.get(config.virtualbox + '/machine/'
...
export async function virtualboxUpdateMachine(name, state, ip = false) {
	try {
		...
		const response = await request.put(config.virtualbox + '/machine/' + encodeURIComponent(name) + (ip ? '?ip' : ''), state);
		const body = await response.json();
		if(response.ok) {
			return body;
		} else {
