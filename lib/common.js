...
fdb.setAPIVersion(config.database.apiVersion);

function setDatabasePrefix(db) {
	if ('prefix' in config.database) {
		return db.at(config.database.prefx);
	}
	return db;
}
export const db = setDatabasePrefix(
	fdb.openSync(config.database.clusterFile).withKeyEncoding(fdb.encoders.tuple)
);
