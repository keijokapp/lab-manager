...
import { authorize, logger, serveRepository } from '../common.js';
...
routes.get('/', apiOperation({
						...
						promises.push(getRepositoryRefs(repository).then(refs => {
							if (refs === null) {
								...
								return {
									_id: repository,
									refs,
									link: `${config.appUrl}/repository/${repository}.git`
								};
							}
						}));
					}
	...
	res.format({
		html() {
			res.send(render('Repositories', { repositories }, '<script src="bundle/repository.js"></script>'));
		},
