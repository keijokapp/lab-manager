...
import { authorize, db, deleteInstance } from '../common';
...
const validateLab = new Ajv().compile(labSchema);
...
routes.get('/', apiOperation({
	...
	res.format({
		html: function() {
			res.send(render(getPathPrefix(req), 'Labs', { labs }, '<script src="bundle/lab.js"></script>'));
		},
...
routes.get('/:lab', apiOperation({
	...
	try {
		...
		res.format({
			html: function() {
				res.send(render(getPathPrefix(req), 'Lab', { lab }, '<script src="bundle/lab.js"></script>'));
			},
