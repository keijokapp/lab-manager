...
import { getPathPrefix } from '../util';
...
routes.post('/:machine/snapshot/:snapshot', apiOperation({
	...
	virtualboxRequest('/machine/' + encodeURIComponent(req.params.machine)
		+ '/snapshot/' + encodeURIComponent(req.params.snapshot), {
		method: 'POST'
	})
		.then(response => {
			if(response.ok) {
