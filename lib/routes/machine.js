...
import { authorize, logger, reqid } from '../common';
...
routes.get('/', apiOperation({
	...
	function getMachines(html) {
		...
		return fetch(config.virtualbox + '/machine?' + params.join('&'), {
...
routes.get('/:machine', apiOperation({
	...
	fetch(config.virtualbox + '/machine/' + encodeURIComponent(req.params.machine) + ('ip' in req.query ? '?ip' : ''), {
		headers: { 'x-request-id': reqid() }
	})
		.then(response => {
			if(response.ok) {
...
routes.post('/:machine/snapshot/:snapshot', apiOperation({
	...
	fetch(config.virtualbox + '/machine/' + encodeURIComponent(req.params.machine) + '/snapshot/' + encodeURIComponent(req.params.snapshot), {
		method: 'POST',
		headers: { 'x-request-id': reqid() }
	})
		.then(response => {
