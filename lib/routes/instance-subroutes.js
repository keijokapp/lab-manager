...
routes.get('/machine/:machine', asyncMiddleware(async (req, res) => {
...
routes.put('/machine/:machine', validate({
	body: {
		type: 'object',
		properties: {
			_rev: { type: 'string', minLength: 1 }, // ignored
			state: { type: 'string', enum: ['starting', 'running', 'stopping', 'poweroff', 'acpipowerbutton'] }
		},
