...
async function machineInfo(type, machine, ip) {
	switch (type) {
	case 'lxd':
		Object.assign(machine, await lxdMachineInfo(machine.name));
		break;
	case 'virtualbox':
		Object.assign(machine, await virtualboxMachineInfo(machine.name, ip));
		break;
	default:
...
routes.use((req, res, next) => {
	if (req.instance && req.instance.imported && !req.instanceImported) {
		const { instance } = req;
		('iTee' in config ? iTeeLabinfo(instance.privateToken) : Promise.resolve(null))
			.then(labinfo => {
				if (labinfo === null) {
					delete req.instance;
			...
			.then(() => {
				logger.debug('Deleted imported instance', {
					instance: instance._id,
					privateToken: instance.privateToken
				});
			}, e => {
				logger.debug('Failed to delete imported instance', {
					instance: instance._id,
					privateToken: instance.privateToken,
					e: e.message
				});
...
routes.get('/', apiOperation({
	...
	if ((req.accepts('html') || 'detailed' in req.query) && 'machineOrder' in instance.lab) {
		const askIp = req.accepts('html') || 'ip' in req.query;
		await Promise.all(instance.lab.machineOrder
			.map(id => machineInfo(instance.lab.machines[id].type, instance.machines[id], askIp)));
	}
...
routes.put('/machine/:machine', apiOperation({
	tags: ['Instance'],
	...
	if (!instance) {
		...
		case 'lxd':
			...
			Object.assign(machine, await lxdUpdateMachine(machine.name, req.body));
			break;
		case 'virtualbox':
			Object.assign(machine, await virtualboxUpdateMachine(machine.name, req.body,
				req.accepts('html') || 'ip' in req.query));
			break;
		}
...
routes.get('/repository/:repository/info/refs', (req, res) => {
	...
	if (!('repositories' in config)) {
		...
		const repositoryLocation = `${config.repositories}/${repository.name}.git`;
		execFile('git-upload-pack', ['--stateless-rpc', '--advertise-refs', repositoryLocation], (e, stdout) => {
			if (e) {
				logger.error('Failed to execute git-upload-pack service', { e: e.message });
				res.status(500).send('Internal Server Error');
			} else if ('head' in repository) {
				execFile('git', ['-C', repositoryLocation, 'show-ref', repository.head], (e, ref) => {
					if (e) {
						logger.error('Failed to resolve head', { e: e.message });
						res.status(500).send('Internal Server Error');
					} else {
...
routes.use('/repository/:repository', (req, res) => {
	...
	if (!instance) {
		...
		const repository = instance.lab.repositories[id];
		serveRepository(req, res, repository.name);
	}
});
