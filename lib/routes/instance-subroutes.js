...
import { asyncMiddleware, getPathPrefix } from '../util';
...
routes.get('/', apiOperation({
	...
	if(req.accepts('html') || 'detailed' in req.query) {
		await Promise.all(instance.lab.machineOrder.map(id =>
			machineInfo(instance.lab.machines[id].type, instance.machines[id],
	...
	res.format({
		html: function() {
			res.send(render(getPathPrefix(req)
				, 'Lab instance'
				, { instance, instanceToken: req.instanceToken }
