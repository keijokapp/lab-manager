...
async function deleteNetworks(instance) {
  if ('machines' in instance) {
    ...
    logger.debug('Deleting dangling networks', {
      networks: danglingNetworks
    ...
    const networkDeletePromise = danglingNetworks.map(network => {
      return lxdRequest('/networks/' + encodeURIComponent(network), {
        ...
        if (e.response instanceof Object && e.error === 'not found') {
          logger.debug('Failed to delete network', {
            network,
            e: 'Not found'
          });
        } else {
...
async function lxdDeleteMachine(name) {
  try {
    ...
    if (e.response instanceof Object && e.error === 'not found') {
      logger.debug('Failed to delete machine', {
        type: 'lxd',
        machine: name,
        e: 'Not found'
      });
...
async function createGitlabGroup(gitlab, publicToken) {
  try {
    ...
    if (response.ok) {
      ...
      logger.error('Failed to create Gitlab group', {
        response: body
      });
    }
  } catch (e) {
...
async function createGitlabUser(gitlab, publicToken) {
  try {
    ...
    if (response.ok) {
      ...
      logger.error('Failed to create Gitlab user', {
        response: body
      });
    }
  } catch (e) {
...
async function iTeeLabinfo(privateToken) {
  try {
    ...
    if (response.ok) {
      ...
      logger.error('Failed to fetch lab instance', {
        response: body
      });
    } else {
      logger.error('Failed to fetch lab instance', {
