...
async function getInstance(labId, userId) {
  ...
  if (ambiguous) {
    _common.logger.error('I-Tee compatibility state', {
      rows: docs.rows
    });
...
async function fetchITeeUser(userId) {
  ...
  if (!response.ok || !Array.isArray(body)) {
    _common.logger.error('Failed to fetch user name from I-Tee', {
      user: userId,
...
async function fetchITeeLab(labId) {
  ...
  if (!response.ok || !Array.isArray(body)) {
    _common.logger.error('Failed to fetch lab from I-Tee', {
      lab: labId,
      status: response.status,
      response: body
    });
...
routes.post('/set_vta_info.json', (0, _expressOpenapiMiddleware.apiOperation)({
  ...
  if (instance) {
    _common.logger.warn('Unable to set VirtualTA info on running lab', {
      id: req.body.id,
  ...
  try {
    ...
    if (e.name === 'conflict') {
      // TODO: replay request
      _common.logger.warn('Conflict updating VirtualTA info', {
        id: iTeeCompat._id,
        rev: iTeeCompat._rev
      });
...
routes.post('/start_lab_by_id.json', (0, _expressOpenapiMiddleware.apiOperation)({
  ...
  if (instance) {
    _common.logger.warn('Unable to start already running lab', {
      id: req.body.id,
      instance: instance._id
    });
  ...
  try {
    ...
    if (e.name === 'not_found') {
      _common.logger.debug('Lab for I-Tee compatibility object was not found', {
        iTeeCompat
      });
...
routes.post('/end_lab_by_id.json', (0, _expressOpenapiMiddleware.apiOperation)({
  ...
  if (!instance) {
    _common.logger.warn('Unable to end stopped lab', {
      id: req.body.id,
      instance: instance._id
    });
...
routes.get('/start_vm.json', (0, _expressOpenapiMiddleware.apiOperation)({
  ...
  if (!machine || !instanceMachine || !machine.enable_restart) {
    ...
    switch (machine.type) {
      case 'lxd':
        result = await (0, _common.lxdUpdateMachine)(instanceMachine.name, {
          state: 'running'
        });
        break;

      case 'virtualbox':
        result = await (0, _common.virtualboxUpdateMachine)(instanceMachine.name, {
          state: 'running'
        });
        break;
    }
